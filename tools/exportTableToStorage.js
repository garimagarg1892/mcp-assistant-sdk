...
async function exportTableToStorage(params) {
  console.log('🌉 Bridge Tool - Received parameters:', JSON.stringify(params, null, 2));
  ...
  if (!params.mysqlDatabase) {
    return {
      success: false,
      message: 'Error: mysqlDatabase is required. Specify which database to export from.',
      receivedParams: params
    };
  }
  ...
  try {
    ...
    console.log(`📊 Retrieved ${mysqlData.rowCount} records from ${params.tableName}`);

    // Step 2: Format the data
    const format = params.format || 'json';
    const formattedData = formatData(mysqlData.data, format, params);
    const fileName = params.fileName || generateFileName(params.tableName, format);

    console.log(`📝 Data formatted as ${format.toUpperCase()}. Size: ${formattedData.length} characters.`);
...
async function queryMySQLMCP(params) {
  return new Promise((resolve, reject) => {
    ...
    console.log('📤 Sending request to MySQL MCP:', JSON.stringify(request, null, 2));
