...
async function putObject(params) {
  console.log('🔍 Universal Upload - Received parameters:', JSON.stringify(params, null, 2));
  ...
  return {
    success: false,
    message: 'Error: Provide either fileContent+fileName, filePath, or directoryPath.',
    receivedParams: params
  };
}
...
async function uploadSingleFile(params) {
  ...
  try {
    ...
    // Determine S3 key
    const s3Key = params.s3Key || params.fileName || path.basename(params.filePath);
...
async function uploadDirectory(params) {
  ...
  try {
    ...
    // Get all files
    const allFiles = await fileSystem.getAllFiles(params.directoryPath, options.fileExtensions);
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      try {
        ...
        if (options.preserveStructure) {
          ...
          s3Key = options.s3Prefix ? `${options.s3Prefix}/${relativePath}` : relativePath;
        } else {
          s3Key = options.s3Prefix ? `${options.s3Prefix}/${fileName}` : fileName;
        }
    ...
    const successRate = totalProcessed > 0 ? (results.successful.length / totalProcessed * 100).toFixed(1) : 0;
